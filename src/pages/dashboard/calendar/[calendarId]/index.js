...
const CustomerList = (props) => {
  ...
  const applyFilters = (customers, filters) => customers.filter((customer) => {
    ...
    if (filters.hasAcceptedMarketing && !(currentEvent.customers.includes(customer.id))) {
      return false;
    }
  
    if (filters.isProspect && !currentEvent.customers_ok.includes(customer.id)) {
      return false;
    }
  
    if (filters.isReturning && !currentEvent.customers_late.includes(customer.id)) {
      return false;
    }
  ...
  const handleSignup = useCallback(async (selectedCustomers) => {
    const event = await calendarApi.addCustomersToEvent({
      eventId: calendarId,
  ...
  const handleSignout = useCallback(async (selectedCustomers) => {
    const event = await calendarApi.removeCustomersFromEvent({
      eventId: calendarId,
      customers:  [...new Set([...selectedCustomers])]
      })
      setCurrentEvent(event)
  ...
  const handleCustomerAssist = useCallback(async (selectedCustomers) => {
    const event = await calendarApi.addAssistCustomersToEvent({
      eventId: calendarId,
  ...
  const handleCustomerLateCancel = useCallback(async (selectedCustomers) => {
    const event = await calendarApi.addLateCustomersToEvent({
      eventId: calendarId,
  ...
  const addCustomerHandler = async () => {
    try {
      ...
      console.log(currentEvent.customers)
